...
class Move {
 public:
  virtual ~Move() {}
  virtual void moveModel(double shift,
                         std::vector<VertexCoordinates> &VertexArray,
                         MaxMinValues &ExtremeValues) = 0;
};

class MoveX : public Move {
 public:
  void moveModel(double shift, std::vector<VertexCoordinates> &VertexArray,
...
class Model_3DViewer : public s21::ContextMove {
 ...
 public:
  ...
  VertexCoordinates FromStringToDouble(std::vector<std::string> numbers_vector,
                                       int &error);
  std::vector<int> FromStringToInt(std::vector<std::string> numbers_vector,
                                   int &error);
  void SetMaxMin();
